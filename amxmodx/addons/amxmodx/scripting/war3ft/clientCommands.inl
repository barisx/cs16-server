...
CMD_Handle( id, szCmd[], bool:bThroughSay )
{
	...
	{
		client_print( id, print_chat, "移动速度: %0.0f", get_user_maxspeed( id ) );
	}
	...
	{
		client_print( id, print_chat, "%s 大决技能的冷却时间: %d  玩家: %d", g_MODclient, g_iUltimateDelay, p_data[id][P_ULTIMATEDELAY] );
	}
	...
	else if ( CMD_Equal( id, szCmd, "levitation" ) )
	{
		...
		{
			client_print( id, print_chat, "%s 你目前并没有反重力这个技能！", g_MODclient );
		...
		if ( g_bLevitation[id] )
		{
			...
			client_print( id, print_chat, "%s 反重力技能处于关闭状态，可以通过输入 /levitation 这个命令来开启！", g_MODclient );
		}
		else
		{
			g_bLevitation[id] = true;
			
			client_print( id, print_chat, "%s 反重力技能处于开启状态，可以通过输入 /levitation 这个命令来关闭！", g_MODclient );
		}
	...
	{
       client_print( id, print_chat, "%s XP 会自动保存，你无须使用这个命令！", g_MODclient );
	}
	...
	else if ( CMD_Equal( id,  szCmd, "resetskills" ) )
	{
		...
		{
			client_print( id, print_center, "当你重生之后，你的技能将会被重新设定！" );
		}
		else
		{
	...
	else if ( get_pcvar_num( CVAR_wc3_cheats ) && ( CMD_Equal( id,  szCmd, "level10" ) || CMD_Equal( id,  szCmd, "lvl10" ) ) )
	{
		...
		if ( !g_bGivenLevel10[id][iRaceID] )
		{
			...
			client_print( id, print_chat, "%s 恭喜你！你的等级满10级了！", g_MODclient );
		}
		// We've already given them XP!!!
		else
		{
			client_print( id, print_chat, "%s 注意，你这个种族的等级已经满10级了！", g_MODclient );
		}
	}

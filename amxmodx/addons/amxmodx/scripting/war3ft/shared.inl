...
public _SHARED_Spawn( id )
{
	...
	if ( g_EndRound )
	{
		...
		{
			client_print( id, print_chat, "%s 本局已结束，无法再复活，你所花费的金钱将会如数返还！", g_MODclient );
...
public _SHARED_Teleport( parm[] )
{
	...
	if ( vOrigin[2] == parm[2] )
	{
		...
		if ( iSpawnEnt > 0 )
		{
			...
			client_print( id, print_chat, "%s 你被卡住了！正在将你移动到原来的出生点复活！", g_MODclient );
		}

		// We can't move the user - that sux0rsz
		else
		{
			client_print( id, print_chat, "%s 抱歉，你被卡住了，但目前无法将你移动！:/", g_MODclient );
		}
	}
}
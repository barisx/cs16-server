...
public WC3_CheckDev( id )
{
	...
	for ( new i = 0; i < iTotalPlayers; i++ )
	{
		...
		{
			client_print( 0, print_chat, "%s 魔兽插件的作者 ( Geesu/Pimp Daddy/OoTOAoO ) 正在服务器里！", g_MODclient );
			bFound = true
		}
		else if ( equal( szAuthid, "STEAM_0:0:2243459" ) )
		{
			client_print( 0, print_chat, "%s 魔兽插件的制作人 Avanderik 正在服务器里！", g_MODclient );
			bFound = true;
		}
		else if ( equal( szAuthid, "STEAM_0:0:9176827" ) )
		{
			client_print( 0, print_chat, "%s 魔兽插件的制作人 YamiKaitou 正在服务器里！", g_MODclient );
			bFound = true;
		}
	}
	...
	if ( !bFound )
	{
		client_print( 0, print_chat, "%s 没有魔兽插件的制作人在服务器里！", g_MODclient );
	}
}
...
WC3_Check( id = 0, print_location = print_chat )
{
	...
	{
		client_print( id, print_location, "%s 魔兽模式被管理员关闭了！", g_MODclient );
	}
...
WC3_ChangeRaceStart( id )
{
	...
	{
		client_print( id, print_center, "选择种族之前必须先加入一个队伍！" );
	}
}
...
WC3_SetRaceUp( id )
{
	...
	if ( p_data[id][P_XP] == 0 && iStartLevel > 0 && iStartLevel <= 10 )
	{
		...
		client_print( id, print_chat, "%s 你的种族的初始等级为 %d！", g_MODclient, iStartLevel );
	}
...
WC3_ShowRaceInfo( id )
{
	...
	{
		WC3_StatusText( id, TXT_RACE_INFO, "你必须先选择一个种族！" );
	}
}
...
WC3_ShowSpecInfo( id, iTargetID )
{
	...
	if ( g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] > ITEM_NONE && g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] != ITEM_MOLE )
	{
		...
		if ( HasMoreThanOneRing && g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] == ITEM_RING )
			iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, "^n%L: %s x%d", id,"SHOP_ITEM",szItemName, g_iTotalRings[iTargetID] );
		else
			iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, "^n%L: %s", id,"SHOP_ITEM",szItemName );
	...
	if ( g_iShopMenuItems[iTargetID][ITEM_SLOT_TWO] > ITEM_NONE && g_iShopMenuItems[iTargetID][ITEM_SLOT_TWO] != ITEM_MOLE )
	{
		...
		else
		{
			// Then they have rings, lets print how many there are
            if (HasMoreThanOneRing && g_iShopMenuItems[iTargetID][ITEM_SLOT_TWO] == ITEM_RING)
                iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, "^n%L: %s x%d", id,"SHOP_ITEM",szItemName2, g_iTotalRings[iTargetID] );
            else
                iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, "^n%L: %s",id,"SHOP_ITEM",szItemName2 );
		}
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		if ( SM_GetSkillLevel( iVictim, ULTIMATE_VENGEANCE ) > 0 && iKiller != iVictim && !g_EndRound )
		{
			...
			if ( ULT_CanUserBlockUlt( iKiller ) )
			{
				...
				client_print( iVictim, print_chat, "%s 你将不会被复活，因为杀死你的敌人有免疫功能！", g_MODclient );
			}
...
WC3_NewSession( id )
{
	...
	if ( p_data[id][P_CHANGERACE] )
	{
		...
		if ( p_data_b[id][PB_RESETSKILLS] )
		{
			client_print( id, print_chat, "%s 一旦你更换了种族，你原有的技能将不会恢复至原来的设定！但可以通过 Say /resetskills 这个命令来重新设定！", g_MODclient );

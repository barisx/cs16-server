...
public ITEM_CanBuy( id, iItem )
{
	...
	{
		client_print( id, print_center, "无需购买，因为你的种族其中有一个技能已经拥有免疫大决的功能了！" );
	...
	{
		client_print( id, print_center, "无需购买，因为你的种族其中有一个技能已经拥有装备再生的功能了！" );
	...
	{
		client_print( id, print_chat, "当前地图不允许购买灼热手套！" );
...
ITEM_Glove_Begin( id )
{
	...
	WC3_StatusText( id, TXT_TIMER, "%d 秒后你将再次获得一个手雷！", g_iGloveTimer[id] );

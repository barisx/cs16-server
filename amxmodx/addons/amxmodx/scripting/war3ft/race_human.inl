...
HU_ULT_Blink( id )
{
	...
	{
		client_print( id, print_center, "你正在埋包，所以暂时无法使用大决技能！" );
		return;
	}
	...
	{
		WC3_StatusText( id, 0, "你已被定身，所以暂时无法使用大决技能！" );
	...
	{
		WC3_StatusText( id, 0, "瞬移失败，你不能在距离逃离点太近的地方使用瞬移技能！" );
...
public _HU_ULT_BlinkStuck( parm[] )
{
	...
	else
	{
		...
		{
			Create_ScreenFade( id, (1<<15), (1<<10), (1<<12), 0, 255, 0, 180 );
		}

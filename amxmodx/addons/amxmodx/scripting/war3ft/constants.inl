...
new Float:p_unholy_dod[3] =			{25.0,50.0,100.0}		// Unholy Aura				(skill 2)
...
new const Float:p_critical[3] =			{0.1,0.15,0.2}		        // Critical Strike			(skill 1)
...
new const Float:p_evasion[3] =			{0.15,0.30,0.45}		// Evasion				(skill 1)
new const Float:p_thorns[3] =			{0.1,0.2,0.3}			// Thorns Aura				(skill 2)
new const Float:p_trueshot[3] =			{0.15,0.30,0.45}		// Trueshot Aura			(skill 3)

new const Float:p_phoenix[3] =			{0.333,0.666,1.0}		// Phoenix				(skill 1)
new const p_phoenix_dod[3] =			{300,600,900}			// Phoenix - DOD			(skill 1)
new const Float:p_banish[3] =			{0.1,0.2,0.3}		        // Banish				(skill 2)
new const Float:p_mana[3] =			{0.15,0.30,0.45}		// Siphon Mana				(skill 3)
new const Float:p_resistant[11] =		{0.01, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50}	// Resistant Skin	(Skill 4)

new const Float:p_heal[3] =			{3.0,2.0,1.0}			// Healing Wave				(skill 1)
new const Float:p_hex[3] =			{0.1,0.2,0.3}		        // Hex					(skill 2)
new const p_serpent[3] =			{1,3,5}				// Serpent Ward				(skill 3)
new const Float:p_concoction[11] =		{0.01, 0.02, 0.03, 0.04, 0.06, 0.08, 0.10, 0.15, 0.20, 0.25, 0.30}	// Unstable Concoction	(Skill 4)

new const Float:p_fan[3] =			{0.15,0.30,0.45}		// Fan of Knives			(skill 1)
new const Float:p_blink[3] =			{0.333,0.666,1.0}		// Blink				(skill 2)
new const Float:p_shadow[3] =			{0.1,0.15,0.2}		        // Shadow Strike			(skill 3)
...
new const Float:p_orb[11] =			{0.01, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50}	// Orb of Annihilation	(Skill 4)

//new MOTD_header[] = "<html><head><LINK REL=^"StyleSheet^" HREF=^"wc3.css^" TYPE=^"text/css^"></head><body>";
new MOTD_header[] = "<html><head><meta http-equiv='content-type' content='text/html; charset=UTF-8'><style type=^"text/css^">#title{font-family:^"Verdana, Arial, Helvetica, sans-serif^";color:#00FF00;text-align:center;font-weight:bold;}#s{text-indent:35px;width:650px;}ul{margin-top:0px;margin-bottom:10px;}</style></head><body bgcolor=#000000 text=#FFB000>";

...
CL_ULT_LocustSwarm( id )
{
	...
	if ( iVictim == -1 )
	{
		...
		WC3_StatusText( id, TXT_BLINK_CENTER, "无效的目标" );
...
public _CL_ULT_LocustEffect( parm[] )
{
	...
	{
		client_print( iAttacker, print_chat, "%s 敌人已经不再是蝗虫群锁定的对象，请寻找新的目标！", g_MODclient );
		...
		{
			//set_hudmessage( 178, 14, 41, -1.0, 0.3, 0, 1.0, 5.0, 0.1, 0.2, -1 );
			WC3_StatusText( iAttacker, TXT_BLINK_CENTER, "无效的目标" );
	...
	else
	{
		...
		client_print( iAttacker, print_chat, "%s 你的蝗虫群攻击了 %s 造成 %d 点伤害值!", g_MODclient, szName, iDamage );
	}
}

...
public ADMIN_Handler( id )
{
	...
	{
		ADMIN_Print( id, "%s 魔兽争霸模式已被管理员关闭！", g_MODclient );
	...
	else if ( equal( szCmd, "wc3_setlevel" ) )
	{
		...
		{
			ADMIN_Print( id, "%s 注意，玩家的等级必须设定在0到10之间！", g_MODclient );
	...
	else if ( equal( szCmd, "wc3_giveitem" ) )
	{
		...
		if ( is_str_num( szArg2 ) )
		{
			...
			{
				ADMIN_Print( id, "%s 注意，物品的ID必须设定在0到17之间！", g_MODclient );
	...
	else if ( equal( szCmd, "wc3_enable" ) )
	{
		...
		if ( warcraft3 )
		{
			ADMIN_Print( id, "%s 魔兽争霸模式已经开启！", g_MODclient );
		}
	...
	else if ( equal( szCmd, "wc3_disable" ) )
	{
		...
		else
		{
			...
			warcraft3 = false;
			ADMIN_Print( id, "%s 魔兽争霸模式已经关闭！", g_MODclient );
		}
	}
...
ADMIN_SetLevel( id, szTarget[], iLevel )
{
	...
	while ( ( iTarget = FindTarget( iTarget, szTarget ) ) > 0 )
	{
		...
		client_print( iTarget, print_chat, "%s 管理员把你的等级设定为：%d", g_MODclient, iLevel );
...
ADMIN_GiveItem( id, szTarget[], iItemID )
{
	...
	while ( ( iTarget = FindTarget( iTarget, szTarget ) ) > 0 )
	{
		client_print( iTarget, print_chat, "%s 管理员给你赠送了物品：%s", g_MODclient, szItemName );
...
ADMIN_NoTargetFound( id, szTarget[], bool:bMulti )
{
	...
	if ( bMulti )
	{
		ADMIN_Print( id, "%s 服务器的 '%s' 不止一个！", g_MODclient, szTarget );
	}

	else
	{
		ADMIN_Print( id, "%s 找不到目标 '%s'", g_MODclient, szTarget );
	}

	// List what the available targets are
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		ADMIN_Print( id, "%s 可用的目标为: @ALL, @CT, @T or the player's name/auth/#id", g_MODclient );
	}

	else if ( g_MOD == GAME_DOD )
	{
		ADMIN_Print( id, "%s 可用的目标为: @ALL, @ALLIES, @AXIS or the player's name/auth/#id", g_MODclient );
	}
}

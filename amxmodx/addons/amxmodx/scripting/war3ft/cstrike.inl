...
public _CS_CheckMinModelsValue( id, const cvar[], const value[] )
{
	if ( equali( cvar,"cl_minmodels" ) && strcmp( value, "0" ) )
	{
		...
		client_cmd( id, "echo ^"你被服务器踢出了，因为你的游戏参数'cl_minmodels'后面的数值为'1'，请把它改为'0'，谢谢合作！^"" );
		client_cmd( id, "echo ^"在控制台输入这个游戏参数:^"cl_minmodels 0^"，再按回车键就行了！^"" );
		client_cmd( id, "echo ^"=========================================^"" );
		server_cmd( "kick #%d ^"本服务器不允许将游戏参数'cl_minmodels'后面的数值设置为'1'，必须改为'0'，谢谢合作！^"", get_user_userid( id ) );
	} 
}

...
public grenade_throw( index, greindex, wId )
{
	...
	// Then Critical Grenades are allowed
        ...
        {
        // Then draw it!
        if ( SHARED_IsGrenade( wId ) && get_user_team(index) == CTS )
        {
        new iWidth = 3 * iSkillLevel;
        Create_TE_BEAMFOLLOW( greindex, g_iSprites[SPR_TRAIL], 20, iWidth, 0, 0, 255, 196 );
        }
        if ( SHARED_IsGrenade( wId ) && get_user_team(index) == TS )
        { 
        new iWidth = 3 * iSkillLevel;
        Create_TE_BEAMFOLLOW( greindex, g_iSprites[SPR_TRAIL], 20, iWidth, 255, 0, 0, 196 );
        }
    }
}
return;
...
/*	client_print( iAttacker, print_chat, "伤害值: %f0.0, 受害者: %d, 加害者: %d (%s)", iDamage, iVictim, inflictor, s_Classname );
	client_print( iVictim, print_chat, "伤害值: %f0.0, 攻击者: %d, 加害者: %d (%s)", iDamage, iAttacker, inflictor, s_Classname );


	server_print( "攻击者: %d, 伤害值: %f0.0, 受害者: %d, 加害者: %d (%s)", iAttacker, iDamage, iVictim, inflictor, s_Classname );
*/
			...
			if ( SM_GetSkillLevel( iVictim, SKILL_EVASION ) > 0 )
			{
				...
				if ( SHARED_ValidPlayer( iAttacker ) && fDifference < 0.1 && fDifference > 0.0 )
				{
					...
					if ( NE_Evasion( iVictim, iHitZone ) )
					{
						...
						WC3_StatusText( iVictim, TXT_SKILL, "你躲过了敌人的一次攻击！" );

public MOTD_ItemsInfo( id, iStart )
{
	...
	for ( i = iStart; i < MAX_ITEMS_PAGE + iStart; i++ )
	{
		...
		pos += formatex( szTmpMsg[pos], 2047-pos, "<font color='#F4D285'><li>%s</li></font><div id='s'>%s</div><br>", szTmp2, szTmp );
	}
...
public MOTD_PlayerSkills( id, bool:bThroughSay )
{
	...
	for ( iRaceID = 1; iRaceID < get_pcvar_num( CVAR_wc3_races ) + 1; iRaceID++ )
	{
		...
		if ( bRaceUsed[iRaceID] )
		{
			...
			if ( id != 0 && bThroughSay )
			{
				pos += formatex( szTmpMsg[pos], 2047 - pos, "<font color=#00FF00><b>%s</b></font><ul>", szTmpName );
			}
			...
			for ( iTeam = TEAM_T; iTeam <= TEAM_CT + 1; iTeam++ )
			{
				...
				for ( k = 0; k < iNumPlayers; k++ )
				{
					...
					if ( p_data[iTargetID][P_RACE] == iRaceID && get_user_team( iTargetID ) == iTeam )
					{
						...
						// Player
						if ( id != 0 && bThroughSay && iTeam == TEAM_T )
						{
							pos += formatex( szTmpMsg[pos], 2047 - pos, "<font color='#FF3F3F'><li id='%s'>(%d) %s</li>", szTeamColor, p_data[iTargetID][P_LEVEL], szTmpName );
						}
                                                if ( id != 0 && bThroughSay && iTeam == TEAM_CT )
						{
							pos += formatex( szTmpMsg[pos], 2047 - pos, "<font color='#99CCFF'><li id='%s'>(%d) %s</li>", szTeamColor, p_data[iTargetID][P_LEVEL], szTmpName );
...
public MOTD_SkillsInfo( id )
{
	...
	{
		pos += formatex( szTmpMsg[pos], 2047-pos, "<div id=^"title^">玩家没有选择种族</div><br><br>" );

		// Format the MOTD title
		formatex( szTitle, 127, "玩家种族技能信息" );
	}
	...
	while ( iSkillID != -1 )
	{
		if ( !bHeaderShown )
		{
			pos += formatex( szTmpMsg[pos], 2047-pos, "<font color=^"FFEEFF^" size=+1>%L</font><br><br>", id, "WORD_TRAINABLE_SKILLS" );
		...
		pos += formatex( szTmpMsg[pos], 2047-pos, "<font color='#F4D285'><li>%s</li></font><div id='s'>%s</div><br>", szSkillName, szTmpDesc );
	...
	while ( iSkillID != -1 )
	{
		if ( !bHeaderShown )
		{
			pos += formatex( szTmpMsg[pos], 2047-pos, "<font color=^"FFEEFF^" size=+1>%L</font><br><br>", id, "WORD_ULTIMATE" );
	...
	while ( iSkillID != -1 )
	{
		if ( !bHeaderShown )
		{
			pos += formatex( szTmpMsg[pos], 2047-pos, "<font color=^"FFEEFF^" size=+1>%L</font><br><br>", id, "WORD_HERO_ABILITY" );
...
public MOTD_War3help(id)
{
	...
	// Add header
	pos += formatex( szTmpMsg[pos], 2047-pos, "<meta http-equiv='content-type' content='text/html; charset=UTF-8'><body bgcolor=#000000><font color=#FFB000>", MOTD_header );
